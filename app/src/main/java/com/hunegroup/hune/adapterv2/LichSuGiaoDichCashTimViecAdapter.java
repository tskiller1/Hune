...
public class LichSuGiaoDichCashTimViecAdapter extends ArrayAdapter<TransactionDTO> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        final ViewHolder vh;
        if (convertView == null) {
            View view = mInflater.inflate(R.layout.item_v2_lichsugiaodichcash, parent, false);
        ...
        if (item.getDefine_transaction() == 1) {
            vh.imvStatus.setImageResource(R.mipmap.ic_increase_red);
        } else if (item.getDefine_transaction() == 2) {
            vh.imvStatus.setImageResource(R.mipmap.ic_decrease_red);
        } else {
            vh.imvStatus.setImageResource(R.mipmap.ic_transaction_red);
        }
    ...
    // Constructors
    public LichSuGiaoDichCashTimViecAdapter(Context context, List<TransactionDTO> objects) {
        super(context, 0, objects);
        this.mInflater = LayoutInflater.from(context);
    }

    public LichSuGiaoDichCashTimViecAdapter(Context context, TransactionDTO[] objects) {
        super(context, 0, objects);
        this.mInflater = LayoutInflater.from(context);
    }
}
