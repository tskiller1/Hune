...
/**
 * Created by tskil on 10/07/2017.
 */
...
public class RadiusOverlayView extends RelativeLayout {
    ...
    protected void createWindowFrame() {
        ...
        paint.setColor(Color.parseColor("#a0d1d4d6")); // This is the color of your activity background
        paint.setStrokeWidth(1);
        osCanvas.drawRect(outerRectangle, paint);
        paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OUT)); // A out B http://en.wikipedia.org/wiki/File:Alpha_compositing.svg
        float centerX = getWidth() / 2;
        float centerY = getHeight() / 2;
        float radius = Math.min(getWidth(), getHeight()) / 2 - 5;
        paint.setStrokeWidth(1);
