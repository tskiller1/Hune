...
public class ThongTinViecLamTuyenDungActivity extends AppCompatActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 101 && resultCode == 200) {
            taskDTO = gson.fromJson(data.getStringExtra(Common.IntentKey.KEY_TASK), TaskDTO.class);
            isChange = true;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v2_thongtinvieclam_tuyendung);
        findViews();
        initData();
    }
    ...
    private void updateViews() {
        ...
        if (taskDTO.getStatus() == 1) {
            ...
            btnAction.setVisibility(View.VISIBLE);
            type_action = 2;
            btnAction.setText(getString(R.string.pay));
        } else {
            tvStatus.setText(R.string.finished);
            tvStatus.setTextColor(getResources().getColor(R.color.green));
            btnAction.setVisibility(View.GONE);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.imv_back:
                if (isChange) {
                    Intent intent = new Intent(ThongTinViecLamTuyenDungActivity.this, DanhSachViecLamTuyenDungActivity.class);
            ...
            case R.id.btn_action:
                if (type_action == 1) {
                    ...
                    SessionType.getInstance().setType(4);
                    if (taskDTO.getOwner_id() == sessionUser.getUserDetails().getId()) {
                        SessionType.getInstance().setId(taskDTO.getUser_id());
                    } else {
                        SessionType.getInstance().setId(taskDTO.getOwner_id());
                    }
                    SessionType.getInstance().setPost_id(taskDTO.getPost_id());
                    SessionType.getInstance().setCategory(taskDTO.getPost().getCategory());
                    Intent intent = new Intent(this, ThanhToanTuyenDungActivity.class);
                    intent.putExtra("task", gson.toJson(taskDTO));
            ...
            if (result != null) {
                ...
                try {
                    ...
                    if (code.contains(Common.JsonKey.KEY_SUCCESSFULLY)) {
                        ...
                        Picasso.with(ThongTinViecLamTuyenDungActivity.this).load(userDTO.getAvatar()).into(imvAvatar);
                    } else {
        ...
        protected void onPostExecute(String result) {
            ...
            if (result != null) {
                ...
                try {
                    ...
                    if (code.contains(Common.JsonKey.KEY_SUCCESSFULLY)) {
                        Toast.makeText(ThongTinViecLamTuyenDungActivity.this, R.string.confirm_information_success, Toast.LENGTH_SHORT).show();
                        Intent intent = new Intent(ThongTinViecLamTuyenDungActivity.this, DanhSachViecLamTuyenDungActivity.class);
                        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                        intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION);
                        startActivity(intent);
