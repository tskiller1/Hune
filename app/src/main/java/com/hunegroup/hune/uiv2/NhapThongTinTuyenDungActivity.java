...
public class NhapThongTinTuyenDungActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v2_nhapthongtin_tuyendung);
        findViews();
        createDialog();
        initData();
    }
    ...
    private void findViews() {
        if (SessionType.getInstance().getSessionType() == 1) {
            dialogQuestion = DialogUtils.dialogQuestion(this, getString(R.string.dialog_create_task_text), new DialogUtils.DialogCallBack() {
                @Override
    ...
    private void dismissScreen() {
        Intent intent = new Intent(NhapThongTinTuyenDungActivity.this, ChiTietUngVienTuyenDungActivity.class);
        ...
        calendar.setOnDateSetListener(new CalendarDatePickerDialogFragment.OnDateSetListener() {
            ...
            public void onDateSet(CalendarDatePickerDialogFragment dialog, int year, int monthOfYear, int dayOfMonth) {
                ...
                if (!myCalendar.getTime().before(today)) {
                    ...
                    Toast.makeText(NhapThongTinTuyenDungActivity.this, R.string.please_select_after_date, Toast.LENGTH_SHORT).show();
                }
            }
        });
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_confirm:
                if (!TextUtils.isEmpty(edSalary.getText())
                    ...
                    String sDateStart = tvTimeStart.getText().toString() + ":00 " + tvDateStart.getText().toString();
                    Date dateStart = Validate.parseStringToDate(sDateStart, "HH:mm:ss dd/MM/yyyy");
                    if (dateStart == null || dateStart.before(today)) {
                        Toast.makeText(NhapThongTinTuyenDungActivity.this, R.string.please_select_after_date, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    String sDateEnd = tvTimeEnd.getText().toString() + ":00 " + tvDateEnd.getText().toString();
                    Date dateEnd = Validate.parseStringToDate(sDateEnd, "HH:mm:ss dd/MM/yyyy");
                    if (dateEnd == null || dateEnd.before(today)) {
                        Toast.makeText(NhapThongTinTuyenDungActivity.this, R.string.please_select_after_date, Toast.LENGTH_SHORT).show();
                        return;
                    }
            ...
            if (result != null) {
                try {
                    ...
                    if (code.contains(Common.JsonKey.KEY_SUCCESSFULLY)) {
                        Intent intent = new Intent(NhapThongTinTuyenDungActivity.this, DanhSachViecLamTuyenDungActivity.class);
