...
import com.hunegroup.hune.adapterv2.LichSuGiaoDichCashTuyenDungAdapter;
...
public class LichSuGiaoDichCashTuyenDungActivity extends AppCompatActivity {
    ...
    LichSuGiaoDichCashTuyenDungAdapter mAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v2_lichsugiaodichcash_tuyendung);
        findViews();
        initData();
    }
                ...
                if (!loading) {
                    ...
                    if (firstVisibleItem + visibleItemCount == totalItemCount && totalItemCount != 0 && totalItemCount < metaDataDTO.getTotal()) {
                        if (metaDataDTO.isHas_more_page()) {
                            ...
                            transactionServer.execute(sessionUser.getUserDetails().getToken(), Common.Type.TYPE_MONEY_CASH, next_page);
                        }
                    } else loading = false;
                }
            }
    ...
    private void initData() {
        ...
        mList = new ArrayList<>();
        mAdapter = new LichSuGiaoDichCashTuyenDungAdapter(this, mList);
        lsvBody.setAdapter(mAdapter);
        loading = true;
        TransactionServer transactionServer = new TransactionServer();
        transactionServer.execute(sessionUser.getUserDetails().getToken(), Common.Type.TYPE_MONEY_CASH, "1");
    }
