...
public class QuanLyHuneAdsThongBaoTuyenDungActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_v2_quanlyhuneads_thongbao_tuyendung);
        findViews();
        initData();
    }
    ...
    private void updateView() {
        ...
        lnlBranch.removeAllViews();
        for (int i = 0; i < item.getAds_branch().size(); i++) {
            View view = LayoutInflater.from(this).inflate(R.layout.item_v2_branch_thongbao_tuyendung, null);
            TextView tvName = (TextView) view.findViewById(R.id.tv_branch_name);
            TextView tvQRCode = (TextView) view.findViewById(R.id.tv_qr_code);
            tvName.setText(item.getAds_branch().get(i).getName());
            final int k = i;
            tvQRCode.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    Intent intent = new Intent(QuanLyHuneAdsThongBaoTuyenDungActivity.this, MaQRTuyenDungActivity.class);
                    intent.putExtra("branch", gson.toJson(item.getAds_branch().get(k)));
        ...
        String location = "";
        for (int i = 0; i < item.getLocation().size(); i++) {
            if (item.getLocation().get(i) == 1) {
                location += getString(R.string.recruiment) + "\n";
            } else {
                location += getString(R.string.find_job) + "\n";
            }
        }
        String numcoupon = item.getTotal_coupon() + " " + getString(R.string.coupon);
