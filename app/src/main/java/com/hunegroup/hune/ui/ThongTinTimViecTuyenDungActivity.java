...
import com.hunegroup.hune.adapter.ThongTinTimViecAdapter_TuyenDung;
...
public class ThongTinTimViecTuyenDungActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private ThongTinTimViecAdapter_TuyenDung thongTinTimViecAdapter_tuyenDung;
    List<Banner> adsList;
    Gson gson = new Gson();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_thongtintimviec_tuyendung);
        findViews();
        AdsServer adsServer = new AdsServer();
        adsServer.execute("3");
        setListThongTin();
                    ...
                    if (firstVisibleItem + visibleItemCount == totalItemCount && totalItemCount != 0 && totalItemCount < metaDataDTO.getTotal()) {
                        if (metaDataDTO.isHas_more_page()) {
                            ...
                            if (SessionLocation.getInstance().isLatLng()) {
                                ...
                                loading = true;
                                getPostRelateServer.execute(sessionUser.getUserDetails().getToken(), "2", SessionLocation.getInstance().getLocation().latitude + "", SessionLocation.getInstance().getLocation().longitude + "", SessionLocation.getInstance().getRadius() + "", next_page);
                            } else {
                                loading = false;
    ...
    private void setListThongTin() {
        ...
        thongTinTimViecAdapter_tuyenDung = new ThongTinTimViecAdapter_TuyenDung(getBaseContext(), thongTinTimViecDTOs);
        listThongTin.setAdapter(thongTinTimViecAdapter_tuyenDung);
        ...
        if (SessionLocation.getInstance().isLatLng()) {
            Log.d("VITRIHIENTAI", sessionUser.getUserDetails().getToken() + " " + SessionLocation.getInstance().getLocation().toString() + " " + SessionLocation.getInstance().getRadius() + "");
            loading = true;
            getPostRelateServer.execute(sessionUser.getUserDetails().getToken(), "2", SessionLocation.getInstance().getLocation().latitude + "", SessionLocation.getInstance().getLocation().longitude + "", SessionLocation.getInstance().getRadius() + "", "1");
        } else {
            loading = false;
            Toast.makeText(getBaseContext(), "Chưa có thông tin về vị trí của bạn", Toast.LENGTH_SHORT).show();
        }
    ...
    public void onClick(View v) {
        ...
        switch (id) {
            ...
            case R.id.imgFilter: {
                startActivity(new Intent(ThongTinTimViecTuyenDungActivity.this, LocThongTinTuyenDungActivity.class));
                break;
            }
            case R.id.btnAdd: {
                startActivity(new Intent(ThongTinTimViecTuyenDungActivity.this, DangTuyenTuyenDungActivity.class));
                break;
            }
        }
    }
                ...
                try {
                    ...
                    if (code.contains(Common.JsonKey.KEY_SUCCESSFULLY)) {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            thongTinTimViecAdapter_tuyenDung.notifyDataSetChanged();
                    ...
                    if (code.contains(Common.JsonKey.KEY_SUCCESSFULLY)) {
                        ...
                        if (adsList.size() > 0) {
                            for (int i = 0; i < adsList.size(); i++) {
                                ImageView imvAds = new ImageView(ThongTinTimViecTuyenDungActivity.this);
                                imvAds.setScaleType(ImageView.ScaleType.FIT_CENTER);
                                Glide.with(ThongTinTimViecTuyenDungActivity.this).load(adsList.get(i).getCover()).into(imvAds);
